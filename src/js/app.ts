...
import {slideDown, slideToggle, generateFile, slideReset, slideUp} from "./utilities";
...
let accounts = [];
let bankDataInput = document.querySelector('#bankDataInput') as HTMLInputElement;
...
bankDataInput.addEventListener('change', () => {
    ...
    let file = bankDataInput.files[0];
...
function generateAccountList(accounts) {
    ...
    const items = detailsContainer.querySelectorAll('.account__item') as NodeListOf<HTMLElement>;
    items.forEach(item => {
